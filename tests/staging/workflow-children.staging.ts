...
beforeAll(async () => {
  ...
  purchaseAgent = await provision({ label: "purchase-agent", role: "sales_agent", staffScope: "purchase" });
  platformAdmin = await provision({ label: "platform-admin", role: "platform_admin", staffScope: "both" });
  ...
  const submit = await seller.client.from("vehicle_opportunities").update({
    status: "envoyee",
    owner_side: "wilmet",
  }).eq("id", oppId).select("status,owner_side,assigned_group").single();
...
test("status history inherits parent visibility", async () => {
  await check("status history parent scope", async () => {
    const sellerHistory = await seller.client.from("opportunity_status_history").select("id").eq("vehicle_opportunity_id", oppId);
    fail(sellerHistory.error, "seller status history");
    expect((sellerHistory.data ?? []).length).toBeGreaterThan(0);

    const staffHistory = await purchaseAgent.client.from("opportunity_status_history").select("id").eq("vehicle_opportunity_id", oppId);
    fail(staffHistory.error, "scoped staff status history");
    expect((staffHistory.data ?? []).length).toBeGreaterThan(0);

    const unrelatedHistory = await unrelated.client.from("opportunity_status_history").select("id").eq("vehicle_opportunity_id", oppId);
...
test("platform admin can create/update information requests and parent-visible actors inherit them", async () => {
  await check("information request parent scope and platform admin", async () => {
    const inserted = await platformAdmin.client.from("information_requests").insert({
    ...
    const updated = await platformAdmin.client.from("information_requests")
    ...
    const sellerRead = await seller.client.from("information_requests").select("id").eq("id", infoId).maybeSingle();
    fail(sellerRead.error, "seller information request read");
    expect(sellerRead.data?.id).toBe(infoId);

    const staffRead = await purchaseAgent.client.from("information_requests").select("id").eq("id", infoId).maybeSingle();
    fail(staffRead.error, "staff information request read");
    expect(staffRead.data?.id).toBe(infoId);

    const unrelatedRead = await unrelated.client.from("information_requests").select("id").eq("id", infoId).maybeSingle();
...
test("partner answers receive database timestamps and become immutable", async () => {
  await check("information request answer audit integrity", async () => {
    const answer = await seller.client.from("information_requests").update({
      status: "answered",
      response: "TM011 seller answer",
      answered_at: "2000-01-01T00:00:00.000Z",
      response_at: "2000-01-01T00:00:00.000Z",
    }).eq("id", infoId).select("answered_at,response_at,response,status").single();
    ...
    expect(new Date(answer.data.answered_at as string).getTime()).toBeGreaterThan(Date.now() - 60_000);
    expect(new Date(answer.data.response_at as string).getTime()).toBeGreaterThan(Date.now() - 60_000);

    const rewrite = await seller.client.from("information_requests")
...
test("internal activity is hidden from seller and visible to scoped staff", async () => {
  await check("internal activity confidentiality", async () => {
    const internal = await platformAdmin.client.from("opportunity_activities").insert({
      vehicle_opportunity_id: oppId,
      kind: "call",
    ...
    const sellerRead = await seller.client.from("opportunity_activities").select("id,body").eq("id", internalActivityId);
    fail(sellerRead.error, "seller internal activity query");
    expect(sellerRead.data ?? []).toHaveLength(0);

    const staffRead = await purchaseAgent.client.from("opportunity_activities").select("id,body").eq("id", internalActivityId).single();
...
test("scoped staff can add activity but seller notes require editable hand-back state", async () => {
  await check("activity write scope", async () => {
    const staffTask = await purchaseAgent.client.from("opportunity_activities").insert({
      vehicle_opportunity_id: oppId,
      kind: "task",
    ...
    const blockedSellerNote = await seller.client.from("opportunity_activities").insert({
      vehicle_opportunity_id: oppId,
      kind: "note",
    ...
    const handback = await service.from("vehicle_opportunities").update({
    ...
    const sellerNote = await seller.client.from("opportunity_activities").insert({
      vehicle_opportunity_id: oppId,
      kind: "note",
    ...
    const sellerActivities = await seller.client.from("opportunity_activities")
      .select("id,kind,body,author_id")
      .eq("vehicle_opportunity_id", oppId);
    fail(sellerActivities.error, "seller activity list");
    expect((sellerActivities.data ?? []).every((row) => row.kind === "note" && row.author_id === seller.id)).toBe(true);
