...
const REQUIRED = ["E2E_SUPABASE_URL", "E2E_SUPABASE_ANON_KEY", "E2E_SUPABASE_SERVICE_ROLE_KEY"] as const;
const EXPECTED_TARGET = "wilmet-staging";
if (process.env.E2E_ALLOW_STAGING_MUTATIONS !== "true") throw new Error("Explicit staging mutation opt-in required.");
if (process.env.E2E_TARGET_LABEL !== EXPECTED_TARGET) throw new Error(`E2E_TARGET_LABEL must be ${EXPECTED_TARGET}.`);
...
async function provision(input: { label: string; role: "partenaire" | "platform_admin"; partnerKind?: "seller" | null }): Promise<Identity> {
  ...
  const created = await service.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { first_name: "Txn", last_name: input.label, partner_kind: input.partnerKind ?? "client" },
  });
  ...
  fail((await service.from("user_roles").delete().eq("user_id", id)).error, `clear ${input.label} roles`);
  fail((await service.from("user_roles").insert({ user_id: id, role: input.role })).error, `assign ${input.label} role`);
...
beforeAll(async () => {
  ...
  const draft = await seller.client.from("vehicle_opportunities").insert({
  ...
  const photos = await seller.client.from("vehicle_photos").insert([
    { vehicle_opportunity_id: oppId, storage_path: `${oppId}/txn-a.png`, is_main_photo: true, sort_order: 0 },
    { vehicle_opportunity_id: oppId, storage_path: `${oppId}/txn-b.png`, is_main_photo: false, sort_order: 1 },
  ]).select("id,sort_order,is_main_photo").order("sort_order");
  ...
  const submit = await seller.client.from("vehicle_opportunities").update({ status: "envoyee", owner_side: "wilmet" })
    .eq("id", oppId).select("status,owner_side").single();
...
test("admin handover and information-request creation commit together", async () => {
  await check("atomic admin handover", async () => {
    const rpc = await platformAdmin.client.rpc("admin_handover_to_partner" as never, {
      p_opportunity_id: oppId,
      p_message: "Atomic handover",
    } as never);
    fail(rpc.error, "admin_handover_to_partner");

    const parent = await service.from("vehicle_opportunities").select("owner_side,status,handover_message").eq("id", oppId).single();
    ...
    const request = await service.from("information_requests").select("id,message,status").eq("vehicle_opportunity_id", oppId).eq("message", "Atomic handover").single();
...
test("seller answer and parent return to Wilmet commit together", async () => {
  await check("atomic seller answer", async () => {
    const request = await service.from("information_requests").select("id").eq("vehicle_opportunity_id", oppId).eq("message", "Atomic handover").single();
    fail(request.error, "load handover request");

    const rpc = await seller.client.rpc("answer_information_request" as never, {
      p_request_id: request.data.id,
      p_response: "Atomic answer",
    } as never);
    fail(rpc.error, "answer_information_request");

    const answered = await service.from("information_requests").select("status,response,answered_at,response_at").eq("id", request.data.id).single();
    ...
    const parent = await service.from("vehicle_opportunities").select("status,owner_side").eq("id", oppId).single();
...
test("admin request-info RPC creates trail and parent status together", async () => {
  await check("atomic admin request info", async () => {
    const rpc = await platformAdmin.client.rpc("admin_request_information" as never, {
      p_opportunity_id: oppId,
      p_message: "Atomic request without handback",
    } as never);
    fail(rpc.error, "admin_request_information");

    const request = await service.from("information_requests").select("id").eq("vehicle_opportunity_id", oppId).eq("message", "Atomic request without handback").single();
...
test("photo reorder is all-or-nothing", async () => {
  await check("atomic photo reorder", async () => {
    fail((await service.from("vehicle_opportunities").update({ owner_side: "partenaire" }).eq("id", oppId)).error, "hand back for reorder");

    const bad = await seller.client.rpc("reorder_vehicle_photos" as never, {
      p_orders: [
    ...
    const unchanged = await service.from("vehicle_photos").select("id,sort_order").in("id", [photoA, photoB]).order("sort_order");
    fail(unchanged.error, "verify failed reorder rollback");
    expect(unchanged.data?.map((row) => row.sort_order)).toEqual([0, 1]);

    const good = await seller.client.rpc("reorder_vehicle_photos" as never, {
      p_orders: [{ id: photoA, sort_order: 11 }, { id: photoB, sort_order: 12 }],
    } as never);
    fail(good.error, "valid reorder");

    const changed = await service.from("vehicle_photos").select("id,sort_order").in("id", [photoA, photoB]).order("sort_order");
...
test("main-photo switch is atomic and a bad target preserves the current main", async () => {
  await check("atomic main photo", async () => {
    const good = await seller.client.rpc("set_main_vehicle_photo" as never, {
      p_opportunity_id: oppId,
      p_photo_id: photoB,
    } as never);
    fail(good.error, "valid main photo switch");

    const selected = await service.from("vehicle_photos").select("id,is_main_photo").in("id", [photoA, photoB]);
    ...
    const bad = await seller.client.rpc("set_main_vehicle_photo" as never, {
      p_opportunity_id: oppId,
      p_photo_id: crypto.randomUUID(),
    } as never);
    if (!bad.error) throw new Error("invalid main photo target unexpectedly succeeded");

    const retained = await service.from("vehicle_photos").select("id,is_main_photo").in("id", [photoA, photoB]);
...
afterAll(async () => {
  ...
  try {
    if (opportunities.length) {
      ...
      if (removed.error) { cleanupFailed = true; console.error(removed.error.message); }
    }
    if (users.length) {
      const roles = await service.from("user_roles").delete().in("user_id", users);
      if (roles.error) { cleanupFailed = true; console.error(roles.error.message); }
      const profiles = await service.from("profiles").delete().in("id", users);
      if (profiles.error) { cleanupFailed = true; console.error(profiles.error.message); }
