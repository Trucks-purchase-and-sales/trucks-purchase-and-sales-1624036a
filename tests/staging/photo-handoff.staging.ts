...
async function provision(input: {
  ...
  fail((await service.from("user_roles").delete().eq("user_id", id)).error, `clear role ${input.label}`);
  fail((await service.from("user_roles").insert({ user_id: id, role: input.role })).error, `assign role ${input.label}`);
...
beforeAll(async () => {
  ...
  const draft = await sellerA.client.from("vehicle_opportunities").insert({
...
test("draft owner can upload photo and create metadata", async () => {
  await check("draft seller photo mutation allowed", async () => {
    ...
    const record = await sellerA.client.from("vehicle_photos").insert({
...
test("submitted Wilmet-owned dossier freezes seller photo mutations and exposes photos to scoped staff", async () => {
  await check("submitted photo integrity and staff parent read", async () => {
    const submit = await sellerA.client.from("vehicle_opportunities").update({
      status: "envoyee",
      owner_side: "wilmet",
    ...
    const blockedUpload = await sellerA.client.storage.from(BUCKET).upload(blockedPath, onePixelPng, {
      contentType: "image/png",
      upsert: false,
    });
    rejected(blockedUpload.error, "seller upload after Wilmet handoff");

    const update = await sellerA.client.from("vehicle_photos")
    ...
    const servicePhoto = await service.from("vehicle_photos").select("sort_order").eq("id", photoId).single();
    fail(servicePhoto.error, "verify frozen photo metadata");
    expect(servicePhoto.data.sort_order).toBe(0);

    const staffRead = await purchaseAgent.client.from("vehicle_photos")
      .select("id")
...
test("Wilmet reclaim blocks seller Storage API deletion", async () => {
  await check("photo Storage delete denied after reclaim", async () => {
    const reclaim = await service.from("vehicle_opportunities").update({
      owner_side: "wilmet",
      status: "envoyee",
...
afterAll(async () => {
  ...
  try {
    if (storagePaths.length) {
      ...
      if (remove.error) { failed = true; console.error(remove.error.message); }
    }
    ...
    if (userIds.length) {
      ...
      if (roles.error) { failed = true; console.error(roles.error.message); }
      const profiles = await service.from("profiles").delete().in("id", userIds);
      if (profiles.error) { failed = true; console.error(profiles.error.message); }
      for (const id of userIds) {
        const removed = await service.auth.admin.deleteUser(id);
        if (removed.error) { failed = true; console.error(removed.error.message); }
      }
    }
  } finally {
