...
6. Une affectation directe ne suffit jamais à elle seule : le compte affecté doit aussi porter un rôle de commercial (`sales_agent` ou `external_agent`).
7. Les helpers pipeline ne sont exécutables que par `authenticated` et `service_role`, pas par `anon`/`PUBLIC`.
...
- `is_external_agent(user)` — rôle `external_agent` uniquement.
- `staff_scope_allows(user, side)` — `profiles.staff_scope` = `side` ou `both`.
- `can_read_pipeline_record(user, assigned_sales_agent_id, assigned_group_id, side)` — accès global autorisé, ou commercial explicitement affecté, ou commercial interne via groupe/périmètre.
...
7. Un utilisateur non commercial dont l'UUID est artificiellement placé dans `assigned_sales_agent_id` n'obtient aucun accès staff par cette affectation seule.
8. Le rôle `anon` ne peut pas exécuter les helpers pipeline privés.
