...
-- 13) Status-history logging trigger. UNTRACKED in git history --
--     migration 20260819005000_workflow_children_parent_scope.sql's own
--     comment says this table is "generated by trusted triggers", but
--     the trigger body itself only ever existed in the live database
--     (same category ADR-005 already named: "status-history logging"
--     was one of the ~13 business-logic-only triggers deliberately
--     excluded from this file's original reconstruction). Discovered
--     missing 2026-08-27 when tests/staging/workflow-children.staging.ts
--     asserted a seller could read their own opportunity's status
--     history and got 0 rows back -- not an RLS gap (the SELECT policy
--     is correct and already reconstructed in section 8), just nothing
--     had ever written a row.
--
--     This is a reasonable-effort RECONSTRUCTION, not a verified-exact
--     copy of the original body (which cannot be recovered) -- built to
--     match the table's own column shape (old_status/new_status
--     nullable-old, not-null-new) and the plan's minimum bar: fire once
--     per actual status change, record who changed it. If the real
--     trigger did more (e.g. also logged a message), that behavior is
--     not reproduced here and isn't needed for any test currently in
--     this repo.
create or replace function public.tg_log_opportunity_status_history()
returns trigger
language plpgsql
security definer
set search_path to 'public'
...
begin
  if new.status is distinct from old.status then
    insert into public.opportunity_status_history (
      vehicle_opportunity_id, old_status, new_status, changed_by_user_id
    ) values (
...
drop trigger if exists trg_log_opportunity_status_history on public.vehicle_opportunities;
create trigger trg_log_opportunity_status_history
after update on public.vehicle_opportunities
for each row execute function public.tg_log_opportunity_status_history();
