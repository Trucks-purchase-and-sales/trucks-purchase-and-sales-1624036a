...
function ProfilePage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    setSaving(false);
    if (error) {
      toast.error(t("profile.toast.saveError"), { description: error.message });
      return;
    }
      ...
      <Card className="mt-6 border-border/70">
        ...
        <CardContent>
          <form onSubmit={save} className="space-y-4">
            ...
            <F l={t("buyer.fields.company")}>
              <Input
                value={form.company_name}
                onChange={(e) => setForm({ ...form, company_name: e.target.value })}
              />
            </F>
            <F l={t("profile.fields.providerType")}>
              <Select
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <F l={t("buyer.fields.email")}>
                <Input value={email} disabled />
              </F>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <F l={t("buyer.fields.city")}>
                <Input
                  value={form.city}
                  onChange={(e) => setForm({ ...form, city: e.target.value })}
                />
              </F>
              <F l={t("buyer.fields.country")}>
                <Input
                  value={form.country}
            ...
            >
              {saving ? t("wizard.saving") : t("profile.save")}
            </Button>
          </form>
        </CardContent>
      </Card>
