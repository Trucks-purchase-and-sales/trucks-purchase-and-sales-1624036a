...
export const Route = createFileRoute("/_authenticated/espace-acheteur")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Vue d'ensemble de vos demandes de véhicules chez Wilmet Trucks.",
      },
...
function BuyerHome() {
  ...
  const rows = (data?.rows ?? []) as any[]; // eslint-disable-line react-hooks/exhaustive-deps, @typescript-eslint/no-explicit-any
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">
            {t("espaceAcheteur.title")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("espaceAcheteur.subtitle")}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <StatTile icon={ListChecks} label={t("espaceAcheteur.kpi.total")} value={summary.total} />
        <StatTile
          icon={Loader2}
          label={t("espaceAcheteur.kpi.open")}
          value={summary.open}
          tint="analysis"
        />
        <StatTile
          icon={CheckCircle2}
          label={t("espaceAcheteur.kpi.closed")}
      ...
      <Card className="border-border/70">
        <CardContent className="space-y-3 p-4">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold">{t("espaceAcheteur.recent.title")}</h2>
            <Button asChild variant="ghost" size="sm">
              <Link to="/mes-demandes">{t("espaceAcheteur.recent.viewAll")}</Link>
            </Button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-border p-6 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {t("espaceAcheteur.recent.empty")}
              </p>
              <Button asChild className="mt-3" size="sm">
                <Link to="/chercher-un-vehicule">{t("espaceAcheteur.recent.createFirst")}</Link>
              </Button>
            </div>
          ) : (
                ...
                <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 p-3">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium">
                      {[r.preferred_brand, r.preferred_model].filter(Boolean).join(" ") ||
                        t("espaceAcheteur.recent.defaultLabel")}
                    </div>
                    <div className="text-xs text-muted-foreground">{r.reference_number ?? "—"}</div>
                  </div>
