  ...
  head: () => ({
    meta: [
      { title: "Chercher un véhicule — Wilmet Trucks" },
      {
        name: "description",
        content:
          "Décrivez votre besoin. Wilmet vérifie ses opportunités disponibles et son réseau pour vous proposer les véhicules les plus pertinents.",
      },
...
function BuyerLeadPage() {
  ...
  useEffect(() => {
    ...
    const resolve = async () => {
      ...
      if (profile.country && /^[a-z]{2}$/i.test(profile.country))
        fill("country", profile.country.toUpperCase());
  ...
  const onSubmit = form.handleSubmit(
    async (data) => {
      ...
      if (auth.status === "loading") {
        toast.error(t("buyer.errors.sessionChecking"));
        return;
      }
      if (auth.status === "authenticated" && auth.partnerKind !== "client") {
        toast.error(t("buyer.errors.notBuyerAccount"));
        return;
      }
      setSubmitting(true);
      const payload = {
      ...
      try {
        ...
        if (auth.status === "authenticated") {
          ...
          if (!sess.session?.access_token) {
            toast.error(t("buyer.errors.sessionExpired"));
            return;
          }
          try {
        ...
        if (!r.ok) {
          ...
          const fallback =
            r.status === 429
              ? t("buyer.errors.rateLimited")
              : r.status >= 500
                ? t("buyer.errors.submit")
                : t("buyer.errors.invalidFields");
    ...
    (errors) => {
      ...
      const firstField =
        fieldOrder.find((f) => (errors as Record<string, unknown>)[f as string]) ??
        (Object.keys(errors)[0] as keyof BuyerLeadInput | undefined);
      const label = firstField
        ? (BUYER_FIELD_LABELS[firstField as string] ?? String(firstField))
        : "";
      toast.error(
        t("buyer.errors.validation", { defaultValue: "Merci de compléter les champs requis." }),
        label
          ? { description: `${t("common.field", { defaultValue: "Champ" })} : ${label}` }
          : undefined,
      );
    },
  );
  ...
  const countryOptions = useMemo(
    () =>
      (ref?.countries ?? [])
        ...
        .map((c) => ({
          value: c.code,
          label: (i18n.language === "fr" ? c.name_fr : c.name_en) || c.name_fr,
        })),
    [ref, i18n.language],
  );
  ...
  const categoryOptions = useMemo(
    () =>
      (ref?.vehicleCategories ?? []).map((c) => ({
        value: c.slug,
        label: (i18n.language === "fr" ? c.label_fr : c.label_en) || c.label_fr,
      })),
    [ref, i18n.language],
  );
  ...
  const brandOptions = useMemo(() => {
    ...
    const allowed = new Set(
      (ref?.categoryBrands ?? [])
        .filter((cb) => cb.category_slug === category)
  ...
  const bodyTypeOptions = useMemo(
    () =>
      (ref?.bodyTypes ?? [])
        .filter(
          (b) => !vehicleType || !b.applies_to || (b.applies_to as string[]).includes(vehicleType),
        )
        .map((v) => ({
          value: v.slug,
          label: (i18n.language === "fr" ? v.label_fr : v.label_en) || v.label_fr,
        })),
      ...
      <main className="container-page py-10 sm:py-14">
        <div className="mx-auto max-w-2xl">
          ...
          <h1
            tabIndex={-1}
            data-step-title
            className="text-3xl font-extrabold tracking-tight sm:text-4xl outline-none"
          >
            {t("buyer.title")}
          </h1>
          ...
          <div className="mt-6">
            <StepProgress step={step} total={STEP_KEYS.length} labels={labels} />
          </div>
              ...
              <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {refError && (
                      <div className="flex items-center justify-between gap-3 rounded-md border border-destructive/40 bg-destructive/5 p-2 text-xs text-destructive">
                        <span>{t("buyer.reference.errorText")}</span>
                        <button
                          type="button"
                          className="underline"
                          onClick={() => void refRefetch()}
                        >
                          {t("buyer.reference.retry")}
                        </button>
                      </div>
                    )}
                    <Field
                      label={t("buyer.fields.vehicleCategory", {
                        defaultValue: "Catégorie de véhicule",
                      })}
                      error={form.formState.errors.vehicle_category?.message}
                      required
                    >
                      <Controller
                    ...
                    <Field
                      label={t("buyer.fields.vehicleType")}
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <SearchableCombobox
                            ...
                            options={(ref?.vehicleTypes ?? []).map((v) => ({
                              value: v.slug,
                              label:
                                (i18n.language === "fr" ? v.label_fr : v.label_en) || v.label_fr,
                            }))}
                            placeholder={t("common.select")}
                          />
                        )}
                          ...
                          render={({ field }) => (
                            <SearchableCombobox
                              ...
                              placeholder={
                                category
                                  ? t("common.select")
                                  : t("buyer.fields.pickCategoryFirst", {
                                      defaultValue: "Sélectionnez d'abord une catégorie",
                                    })
                              }
                            />
                          )}
                          ...
                          render={({ field }) =>
                            modelOptions.length > 0 ? (
                              ...
                              <Input
                                ...
                                placeholder={
                                  brandSlug
                                    ? t("buyer.fields.freeText")
                                    : t("buyer.fields.pickBrandFirst")
                                }
                              />
                            )
                          }
                ...
                {step === 1 && (
                  <div className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                      <Field
                        label={t("buyer.fields.minYear")}
                        error={form.formState.errors.min_year?.message}
                      >
                        <Controller
                          name="min_year"
                          control={form.control}
                      ...
                      <Field
                        label={t("buyer.fields.maxMileage")}
                        hint={t("buyer.fields.maxMileageHint")}
                        error={form.formState.errors.max_mileage?.message}
                      >
                        <Input
                          inputMode="numeric"
                          type="number"
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <SearchableCombobox
                              value={field.value || undefined}
                              onChange={field.onChange}
                              options={(ref?.euroStandards ?? []).map((e) => ({
                                value: e.slug,
                                label: e.label,
                              }))}
                              placeholder={t("common.select")}
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <SearchableCombobox
                              value={field.value || undefined}
                              onChange={field.onChange}
                              options={(ref?.gearboxTypes ?? []).map((v) => ({
                                value: v.slug,
                                label:
                                  (i18n.language === "fr" ? v.label_fr : v.label_en) || v.label_fr,
                              }))}
                      ...
                      <Field
                        label={t("buyer.fields.ptac")}
                        hint={t("buyer.fields.ptacHint")}
                        error={form.formState.errors.ptac_kg?.message}
                      >
                        <Input
                          inputMode="numeric"
                          type="number"
                          min={0}
                          {...form.register("ptac_kg", {
                            setValueAs: (v) =>
                              v === "" || v === null || v === undefined ? null : v,
                          })}
                        />
                      ...
                      <Field
                        label={t("buyer.fields.payload")}
                        hint={t("buyer.fields.payloadHint")}
                        error={form.formState.errors.payload_kg?.message}
                      >
                        <Input
                          inputMode="numeric"
                          type="number"
                          min={0}
                          {...form.register("payload_kg", {
                            setValueAs: (v) =>
                              v === "" || v === null || v === undefined ? null : v,
                          })}
                        />
                    ...
                    <Field
                      label={t("buyer.fields.requiredEquipment")}
                      hint={t("buyer.fields.requiredEquipmentHint")}
                    >
                      <Controller
                        ...
                        render={({ field }) => (
                          <MultiSelectBadges
                            value={field.value ?? []}
                            onChange={field.onChange}
                            options={(ref?.equipment ?? []).map((e) => ({
                              value: e.slug,
                              label:
                                (i18n.language === "fr" ? e.label_fr : e.label_en) || e.label_fr,
                            }))}
                          />
                        )}
                      />
                    ...
                    <Field
                      label={t("buyer.fields.wantedEquipment")}
                      hint={t("buyer.fields.wantedEquipmentHint")}
                    >
                      <Controller
                        name="wanted_equipment"
                        control={form.control}
                        render={({ field }) => (
                          <MultiSelectBadges
                            value={field.value ?? []}
                  ...
                  <div className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-3">
                      <div className="sm:col-span-2">
                        <Field
                          label={t("buyer.fields.maxBudget")}
                          hint={t("buyer.fields.maxBudgetHint")}
                          error={form.formState.errors.max_budget_ht?.message}
                        >
                          <Input
                            inputMode="decimal"
                            type="number"
                          ...
                          render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="selon_opportunite">
                                  {t("buyer.choices.flexOpp")}
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          )}
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                <SelectItem value="immediat">
                                  {t("buyer.choices.tImmediat")}
                                </SelectItem>
                                ...
                                <SelectItem value="projet_futur">
                                  {t("buyer.choices.tFutur")}
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          )}
                          ...
                          render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="a_discuter">
                                  {t("buyer.choices.finDiscuss")}
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          )}
                  ...
                  <div className="space-y-4">
                    {auth.status === "authenticated" && auth.partnerKind === "client" && (
                      <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                        {t("buyer.authNotice.connectedAs")}{" "}
                        <span className="font-medium text-foreground">{auth.email}</span>{" "}
                        {t("buyer.authNotice.trackedNote")}
                      </p>
                    )}
                    {auth.status === "authenticated" && auth.partnerKind !== "client" && (
                      <p className="rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-xs text-destructive">
                        {t("buyer.errors.notBuyerAccount")}
                      </p>
                    )}
                    <RequestRecap
                      form={form}
                      labels={{
                        category: categoryOptions.find(
                          (o) => o.value === form.watch("vehicle_category"),
                        )?.label,
                        type: (ref?.vehicleTypes ?? []).find(
                          (v) => v.slug === form.watch("vehicle_type"),
                        )?.label_fr,
                        body: bodyTypeOptions.find((o) => o.value === form.watch("body_type"))
                          ?.label,
                      }}
                    />
                    <div className="grid gap-4 sm:grid-cols-2">
                      <Field
                        label={t("buyer.fields.firstName")}
                        required
...
function RequestRecap({
  ...
  const rows: [string, string | null][] = [
    [
      t("catalog.vehicle"),
      [labels.category, labels.type, labels.body].filter(Boolean).join(" · ") || null,
    ],
    [
      t("buyer.recap.brandModel"),
      [v.preferred_brand, v.preferred_model].filter(Boolean).join(" ") || null,
    ],
    [
      t("buyer.recap.criteria"),
      [
        v.min_year ? t("buyer.recap.fromYear", { year: v.min_year }) : null,
        num(v.max_mileage, t("buyer.recap.kmMaxSuffix")),
        v.min_euro_norm || null,
        v.fuel_type || null,
        v.gearbox || null,
        num(v.ptac_kg, t("buyer.recap.ptacSuffix")),
        num(v.payload_kg, t("buyer.recap.payloadSuffix")),
      ]
        .filter(Boolean)
        .join(" · ") || null,
    ],
    [
      t("buyer.steps.budget"),
      [
        num(v.max_budget_ht, `${v.currency ?? "EUR"} ${t("buyer.recap.htMaxSuffix")}`),
        v.buy_timeline || null,
        v.financing_needed ? t("buyer.recap.financing", { value: v.financing_needed }) : null,
      ]
        .filter(Boolean)
        .join(" · ") || null,
    ],
